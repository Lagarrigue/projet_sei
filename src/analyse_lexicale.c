...
L_LEXEME analyse_lexicale(char* nom_fichier){
	...
	/* i,j,k sont des compteur */
        ...
        while(fgets(ligne,511,fichier) != NULL) /* on lit une nouvelle du fichier texte */
        {
            ...
            while(ligne[i]!='\n') /* on lit la ligne mise en mémoire temporaire caractère par caractère */
            {
                ...
                case REGISTRE: 
                    ...
                    if (ligne[i]=='\n') /*gestion du cas particulier d'ajout de lexeme en cas de fin de ligne*/
                    {
                        j=0;
                        /*strcpy(lexeme_ligne.valeur, mot);*/
                        copie_tab_char(mot, lexeme_ligne.valeur,512);
                        ...
                        valeur_registre=atoi(mot);
                        if(valeur_registre>31) /*gestion d'erreur numero de registre dans ce cas*/
                        {
                            printf("erreur syntaxe ligne n° %d : registre inconnu\n", numero_ligne_programme);
                        }
		    ...
		    if (ligne[i]=='\n') /*gestion du cas particulier de fin de ligne pour ajouter le lexeme de type directive*/
                    {
                        j=0;
			lexeme_ligne.nom_type=6;
                        /*strcpy(lexeme_ligne.valeur, mot);*/
                ...
                case SYMBOL_ALPHA_NUM:  /*ici on ajoute dans un lexeme tous les caractère alpha_numériques puis on ajoute dans la liste de lexeme, le type (alpha, num, hexa, ou étiquette) sera redéfinit plus tard*/
                    ...
                    if (ligne[i]=='\n')/*gestion du cas particulier d'un '\n' comme plus haut*/
                    {
                        j=0;
			lexeme_ligne.nom_type=10;
                        /*strcpy(lexeme_ligne.valeur, mot);*/
